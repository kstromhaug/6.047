...
def viterbi(X):
    ...
    The returned Y is a list of integers, 0=High-GC, 1=Low-GC. <-- j
    """
    # Y is the list of all the states, sequence of states that generates the highest probability
    # X is the input, 
    N = len(tr)
    L = len(X)
    assert len(em) == N
    ...
    for i in xrange(0,L):
        ...
        for k in xrange(N): # in range 2 because two states
            best_j = None
            ov_max = -float('inf')

            for j in range(N):
                trans_prob = Vprev[j] + log(tr[j][k])
                if trans_prob > ov_max:
                    best_j = j
                    ov_max = trans_prob

            emission = em[k][X[i]]

            V[i][k] = log(emission)+ov_max
            TB[i][k] = best_j


            #mx = emission * max(Vprev[k]* tr[k][j] for j in range(len(Vprev)))
